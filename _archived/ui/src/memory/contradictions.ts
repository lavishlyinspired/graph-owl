...
/** Why a pair was flagged, as the server names it. */
export type ContradictionKind = "confirmed" | "declared" | "candidate";

export interface Contradiction {
  /** The smaller memory id — the pair is unordered, and `a`/`b` are only a
   *  canonical ordering so the same two memories always form the same pair. */
  readonly a: string;
  readonly b: string;
  /** The asset both are anchored to, for a candidate. Absent for a declared
   *  contradiction, which needs no shared subject to be real. */
