#!/usr/bin/env bash
# Epic 39 Slice F: the first-run journey, against a real server and a real,
# genuinely empty Postgres — the same reason `verify-generated-client.sh`
# exists for the generated API client: a UI test that never talks to the real
# backend cannot catch what only shows up when it does.
#
#   1. build the console (graph-owl-ui embeds ui/dist at Rust compile time)
...
(cd ui && GRAPH_OWL_BASE_URL=http://127.0.0.1:8099 npx playwright test)
