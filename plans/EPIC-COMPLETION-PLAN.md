...
|---|---|---|---|---|
| 3.1 | 35 | **Activity feed has no `AccessPredicate` enforcement** — a caller can see thread/proposal/change-event activity on entities they cannot otherwise read. The plan itself names this "the one gap that is a real correctness question, not a convenience cut." | small (<1 day) but **treat as priority**, not cosmetic — it's an authorization hole | Thread the existing `AccessPredicate` pattern (already used in Epic 9a's `project_incremental`) into the feed's merge query |
