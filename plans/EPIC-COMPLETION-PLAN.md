...
| 1.9 | 97 | ~~`run_reasoning_incremental(retracted, budget)` has zero callers outside tests — `/reasoning/runs` always does a full run. DRed never executes against a live deployment.~~ **DONE — two layers.** An explicit caller-supplied path was wired early (before 4.4 was finalized; `crates/graph-owl-server/tests/reasoning_incremental.rs`, DEMOS.md previously mis-attributed this to decision 4.4 and has been corrected). **The actual decision 4.4 (server-tracked watermark) implemented 8 Aug 2026**: `Catalog::run_reasoning_auto` computes `retracted` automatically via a new `TripleStore::retractions_since` and calls it — `POST /reasoning/runs` uses this whenever the body has no explicit list. See 3.11's row for the shared implementation. | medium, gated on 4.4 | `crates/graph-owl-server/src/lib.rs:7084-7092`; `crates/graph-owl-api/src/lib.rs` (`run_reasoning_auto`) |
...
| 3.11 | 97 | ~~`maintained_to` freshness stamp on `ReasoningReport`~~ **DONE 8 Aug 2026, built together with decision 4.4** — reserved (`TripleStore::next_time`) *before* the run reads its base, not after: a retraction landing mid-run stays visible to the *next* call's `retractions_since` rather than being silently treated as already accounted for. `Catalog::reasoning_cache`'s type changed from `Option<Reasoning>` to `Option<CachedReasoning>` (a small private wrapper: `{result, maintained_to}`) so the watermark travels alongside the structured previous result `derive_incremental` needs. Reaches the wire as `maintainedTo` on every `/reasoning/runs` response. **The critical test**: `the_reasoning_overlays_own_churn_is_never_mistaken_for_a_base_retraction` — `finish_reasoning_run` retracts the *previous* run's own overlay before writing a new one, a real retraction in the same flake log; without filtering it out by `cx`, every `run_reasoning_auto` call after the second would wrongly see its own prior churn as a base retraction and go Incremental forever, never Full. Manually verified this test fails without the filter (reverted a copy of the filter, watched it fail with exactly the predicted `Incremental` vs. expected `Full`, restored it). Mutation-tested across all three touched crates (`graph-owl-engine`'s new default trait method, `graph-owl-engine-postgres`'s real Postgres query, `graph-owl-api`'s `run_reasoning_auto`/watermark threading) — see 1.9's row for the shared HTTP wiring and `crates/graph-owl-server/tests/reasoning_auto.rs` for the end-to-end proof against real Postgres. | small (<1 day) once 1.9/4.4 lands | Epic 98's EL cache already has an identical watermark pattern to copy |
...
| 3.15 | 42 | ~~Export dialog (`ExportDialog.tsx`)~~ **DONE 9 Aug 2026** — `Catalog::authorized_flakes_scoped`/`authorized_lpg_elements_scoped` gained `scope`/`as_of`, threaded through all 6 export routes plus a new `GET /graph/export/preview`; `ExportDialog.tsx` offers all 9 formats (RDF's 4 wire formats each their own entry) with scope/as-of/preview, downloading via `fetch`+`Blob` since export routes are Bearer-authenticated and a plain `<a href>` would 401. Found and fixed a real bug via this slice's own RED test: `TripleStore::time_at` returning `None` ("nothing existed yet") was being collapsed into "unbounded", so `?asOf=1970-...` silently returned everything instead of nothing — fixed by mapping to `i64::MIN`, reusing the existing `t <= as_of` bound rather than a per-format empty-response branch. Mutation-tested 0 missed on both crates; `exportFormats.ts` 100% Stryker score. Preview is a count, not "first records" — narrower than the original spec, recorded as such. | medium-large | shared scope with Epic 94's RDF-export gap (1.8) — plan together |
...
| 5.2 | 14 | ~~Full outbound webhook feature~~ **DONE, both slices, 9 Aug 2026.** Slice A: registrations + delivery-queue persistence, admin HTTP endpoints at `/admin/outbound-webhooks`, real `EventSink` wiring in `main.rs`. Slice B: `OutboundWebhookSender` (spawned in `main.rs`) polls `list_pending_outbound_webhook_deliveries` every 2s, re-checks `admit_target`, signs and sends via `reqwest`, applies `backoff`'s six-attempt schedule or dead-letters, and `GET /admin/outbound-webhooks/{id}/deliveries` gives operator visibility. See DEMOS.md's Epic 14 section for the full writeup | done | 4.2 (satisfied) |
...
| 15 | 5.1 | 39 | 2.11 + 5.8 |
| 16 | 2.7 + 4.3 (needs decision for full scope) | 42 | 3.2 + 3.14 + 2.12–2.14 + 3.15 — all DONE 9 Aug 2026 |
| 19 | 2.8 | 94 | 1.8 |
