...
| 5.2 | 14 | ~~Full outbound webhook feature~~ **Slice A shipped 9 Aug 2026** (registrations + delivery-queue persistence, admin HTTP endpoints at `/admin/outbound-webhooks`, real `EventSink` wiring in `main.rs`) — see the epic's own "Slice A" note below. **Slice B (the background sender that drains the queue and makes the HTTP request) remains**: medium, 1-2 days | 4.2 (satisfied) |
