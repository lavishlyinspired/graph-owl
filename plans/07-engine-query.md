...
7. **The target is SPARQL 1.1, and 1.2 changes nothing yet.** SPARQL 1.2 Federated Query is at Candidate Recommendation (7 April 2026); the Query Language, Protocol and Entailment documents are Working Drafts. Since federation is explicitly out of scope for this subset, the one part of 1.2 that is nearly stable is the one part this epic does not implement — so 1.1 is the target and there is no churn to accept. The 1.2 change that *would* matter is triple-term patterns.

   **Update, 28 July 2026 — the decider has decided, and it moves the parse surface.** `94-rdf12-alignment.md` decision 7 and Slice D are now scheduled, and building them requires the `sparql-12` Cargo feature on `spargebra` (cascading to `oxrdf/rdf-12` and `spareval`). That feature is crate-wide and compile-time: it turns on **all** of SPARQL 1.2's syntax at once. So "the target is SPARQL 1.1" survives as a statement about **what this engine evaluates**, and stops being true of **what it parses**. The two were the same thing when this decision was written and are not any more; keeping the sentence without the distinction would make it quietly false. The new obligation is Slice D's: a 1.2 construct that parses but is not evaluated must return a named refusal, never an internal error, because "parses then fails confusingly" is strictly worse than today's clean parse error. **That set is smaller than it first appears** — `spareval` implements `LangDir`, `StrLangDir` and `HasLang` under the same flag, and evaluates triple-term patterns rather than only parsing them, so the refusal list is to be *measured* once the flag is on rather than assumed from the syntax list.

   **Landed 7 August 2026, one slice earlier than this update predicted.** The trigger was Slice B, not Slice D: `graph-owl-rdf-io` needs `oxrdf/rdf-12` for Turtle export (`Term::Triple`), `graph-owl-rdf-io` depends on `graph-owl-query`, and Cargo unifies features for one crate+version across a shared build graph — so `spargebra`'s and `spareval`'s own `sparql-12` had to come on in the same commit, or their *own* internal `Term`/`TermRef` matches (not this project's code — `sparesults`, `oxttl`'s N3/TriG modules, `spargebra::term`) failed to compile. Measured, not assumed: enabling only `oxrdf/rdf-12` directly was tried first and does *not* work — each dependent crate's own feature of the same name has to be requested too, since a dependent's `#[cfg(feature = "...")]`-gated conversions stay compiled out otherwise, against a `Term` that now has a variant they were never told to expect. `graph_owl_query::term::from_term` and `pushdown::named` both gained a named `Term::Triple` arm as a direct result (refused, resp. non-narrowing) — Slice D's own "measured refusal list", `dataset.rs` synthesis, and RED tests remain unbuilt; only the compile-time surface moved early.

   **Triple-term patterns were OUT of this epic, and `94-rdf12-alignment.md` decided when they come in.** Naming the decider matters, because this decision previously deferred to Epic 9 and *`09-engine-rdf-io.md` decision 4 deferred straight back* — its revisit trigger is "a SPARQL query needs to match on triple-term patterns directly", which is this epic. Two decisions each waiting on the other is not a deferral, it is a decision nobody owns, and it stayed invisible because each half reads as reasonable alone. Epic 94 owns the vocabulary and both others defer *through* it, so it is the only one that can break the tie. See its decision 7 for the resolved shape.

   **What "out" costs today, stated plainly**: a query written against the standard vocabulary — `?rel rdf:reifies << ?a ?p ?b >>` — returns **zero rows**, not an error. `rdf:reifies` appears nowhere in `graph-owl-core` or `graph-owl-query`; the store uses `dsc:fromEntity` / `dsc:toEntity` / `dsc:relType`. Zero rows is indistinguishable from an empty graph, which is the failure this project treats as worst. Recorded here so the cost of "out" is a known position rather than a surprise.
