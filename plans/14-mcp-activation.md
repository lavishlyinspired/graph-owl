...
**Status**: **Shipped, 9 August 2026.** Slices A–G built (F's registration/enqueue and its background sender both shipped 9 August 2026 — see this file's own Slice F entry and `DEMOS.md`'s Epic 14 section). **Slice H — stdio transport, `rust-mcp-schema` → `rmcp` swap — shipped 9 August 2026.** `POST /mcp` now speaks `rmcp`'s protocol-version type (behaviour-identical: both crates' `latest()`/`LATEST` resolve to the same `2025-11-25`); a second, independent transport (`graph-owl-mcp-stdio`, a new binary) dispatches through the identical `jsonrpc::handle` path stdio and HTTP always shared, with its own principal resolved once at process start rather than per request. See this file's own Slice H section for the full account.
...
### Slice H: stdio transport, and the schema crate swap from `rust-mcp-schema` to `rmcp` — **shipped, 9 August 2026**

**Value**: Closes the transport gap that matters most — stdio is how most MCP clients (Claude Desktop, most IDE integrations) actually connect, and `POST /mcp` reaches none of them. Also replaces a schema crate that has fallen behind the deployed spec with the one `00j-language-boundaries.md` named as the reason MCP could stay Rust in the first place.
**Path**: `graph-owl-mcp`, `rmcp` (`default-features = false`, `features = ["macros", "server", "transport-io"]` — `server` implies `schemars`, so it is not named separately; `server` also turned out to be required for `transport-io`'s `stdio()` function to be reachable at all, found only by compiling, not by reading the feature table) in place of `rust-mcp-schema` for protocol types. `graph-owl-server` gained a matching `rmcp` dependency (`server` + `transport-io`) and a new binary, `graph-owl-mcp-stdio`, plus `src/stdio.rs`'s `run_session` — the newline-delimited-JSON framing loop, generic over `AsyncBufRead`/`AsyncWrite` so it is unit-testable against in-memory buffers without a real process. `POST /mcp` is untouched — a second, independent transport onto the same `jsonrpc::handle` dispatch, not a rewrite of the first.
**Acceptance criteria**: a client speaking stdio can `initialize`, list all 13 tools, and call both a read and a write tool, with authorization enforced identically to the HTTP transport for an equivalent principal; stdio's principal resolution is explicit (a single service-account credential supplied at process start, since stdio has no per-request header to carry one) and **defaults closed** — an unconfigured stdio server refuses every tool call rather than running open; `ProtocolVersion` now comes from `rmcp`, not `rust-mcp-schema`, with a test asserting it is not a hardcoded fallback; the HTTP transport's existing test suite still passes unmodified, since the schema swap must not change wire behaviour observable to HTTP clients.
**RED**: A parity test sending the same `tools/call` request over both transports and asserting structurally identical responses (allowing only transport-framing differences) — the risk of a second transport is silent divergence from the first, particularly on authorization. A second test asserting a stdio server started without a configured principal refuses every call rather than defaulting open. Mutator watch: a stdio call that skips policy filtering, or an unconfigured server that answers instead of refusing, must fail these tests.
**Done when**: criteria met, mutation report reviewed, commit approved.

**Shipped.** `rmcp::model::ProtocolVersion::LATEST` and `rust_mcp_schema::ProtocolVersion::latest()` both resolve to the identical `2025-11-25` — the swap is behaviour-neutral for the one value that matters today, verified by running the HTTP transport's existing test suite unmodified rather than assumed. `LATEST`, not `rmcp`'s newer `STANDARD_HEADERS` (2026-07-28, SEP-2243) — this server does not implement that revision's stateless-HTTP/MRTR/subscriptions surface, and advertising it would be exactly the false-conformance claim the pin exists to prevent.

**"Defaults closed" needed no new authorization logic.** `jsonrpc::handle` already refuses every tool call when its `principal` argument is `None` (Slice A's own property, proven since that slice). An unconfigured stdio session simply never has anything to pass but `None` for `who` — there is no second gate to build or get wrong, only a single line deciding what `who` is.

**A real gap found while writing the "configured session succeeds" test, not designed in advance**: a freshly-configured `GRAPH_OWL_MCP_STDIO_PRINCIPAL` auto-provisions with no roles, and authorization denies by default — the identical "successful sign-in, empty catalog" trap `is_bootstrap_admin` exists to solve for HTTP's own OIDC sign-in path. Fixed by reusing that exact function (`graph_owl_server::is_bootstrap_admin`) against `GRAPH_OWL_ADMIN_SUBJECTS`, rather than inventing a second bootstrap-admin mechanism for this transport — an operator names a stdio agent's id in the same variable they would for an HTTP principal.

**Also found while writing that same test**: the reserved `"system"` username is not a shortcut to the in-process `Principal::system()` admin bypass when resolved through `resolve_principal` — `V15` seeds it deliberately with `is_admin = false` (attribution, not authorization), so a naive test configured with `GRAPH_OWL_MCP_STDIO_PRINCIPAL=system` gets a real, unprivileged principal and "no such entity" for everything, correctly. This is the intended, secure behaviour — an operator cannot self-grant elevated privilege by naming a stdio principal `system` — recorded here because it read as a bug on first sight and cost real investigation before it was understood as correct.

Verified: 9 unit tests in `stdio.rs` (framing: EOF, multi-line ordering, blank-line skipping, malformed-line recovery, notification silence, the defaults-closed/configured pair) against a local fixture, no real process; 3 integration tests in `tests/mcp_stdio.rs` against real Postgres — the parity test (same call, same underlying `Catalog`, both transports, asserting genuine success rather than a vacuous match on two equal failures), the real compiled binary refusing unconfigured, and the real compiled binary answering when configured (which also proves logging stayed off stdout — every test's first stdout line parses as the JSON-RPC response, never a log line). Mutation-tested clean across all three touched files: `jsonrpc.rs`'s diff (2/2 caught), `stdio.rs` (1 caught, 1 timeout — the timeout mutant provably introduces an unbounded read loop on EOF, the correct outcome for that mutation rather than a coverage gap), and the binary's own decision logic (1/1 caught, run against the real integration test since this is a thin imperative shell no `--lib` run reaches).
