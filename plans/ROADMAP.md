...
| **Hosting large external ontologies (SNOMED, UMLS, DBpedia) at 10⁸–10⁹ triples** | **Under review, 28 Jul 2026 — this is a positioning fork, not a deferral.** A requirement was stated for it. `00n-large-ontology-reality.md` sets out what it costs: OWL 2 RL **cannot classify SNOMED CT** (RL and EL are incomparable, so the result is wrong rather than partial), the reasoning overlay's wholesale-replacement design fails arithmetically above ~10⁸, and the Epic 6 budgets are calibrated three orders of magnitude low. Epics 97, 98 and 100 have had their triggers fire; Epic 104 was created for the uncovered half | Adopting it is a `00a-product-position.md` change and belongs there — not a roadmap reshuffle |
| **Whole-term dictionary encoding of flakes** | Added 28 Jul 2026 after an indexing review. Namespaces are *already* encoded to `INTEGER` (u16-ranged) against the compile-time and runtime registries; only the local part is `TEXT`. The proposal that prompted this — one dictionary table with every term a foreign key — would collapse the namespace/local split and discard the predicate registry Epic 4 Slice H made enforceable on write, which is a regression wearing an optimisation's clothes. See `04-engine-triples.md`, "Indexing review" | Encoding the **local part only**, if `37a-scale.md` measures index width or seek time as an actual limit. A measurement, never the observation that other stores do it |
| **Content-addressed index storage, novelty-overlay writes, snapshot-isolation overlays** | Proposed as adaptations of a named source-available reference with a non-compete. `00i-licensing.md` rule 3 forbids adopting even *adapted* work from it, and `CLAUDE.md` records that this failure mode has already occurred once during planning. Independently of licensing, none is justified at catalog scale | Only from the specification or a licence-checked permissive implementation, justified by graph-owl's own measurements |
