...
| 27 | **Public API stability policy (Epic 37c Slice D): pre-1.0 SemVer, enforced by `#![deny(missing_docs)]` rather than a stability-tier annotation system** | Decision 5 in `37c-embeddable.md` commits to SemVer "from the first published crate", but every publishable crate (`core`, `storage-memory`, `api`) is `0.1.0` — under SemVer, a `0.y.z` crate may break on any minor bump, so the policy *is* the SemVer spec read straight: patch (`0.1.x`) is additive/fix-only, minor (`0.x.0`) may break. There is no `#[unstable]`/`#[doc(hidden)]` tiering, because every `pub` item in `core`/`api` is already held to the same bar by `#![deny(missing_docs)]` — a second tiering system would let an item be "public but unstable" without the compiler tracking which, which is exactly the gap `deny(missing_docs)` exists to close for documentation. `1.0.0` is not cut until Slice F proves the surface survives a second entity family | A crate reaches API maturity an embedder can rely on across minor bumps, at which point `1.0.0` and conventional SemVer (breaking only on major) apply |
| 28 | **`AssetKind::parent_kind()` stays a single-parent chain; Epic 34 Slice E special-cases a second valid parent at the call site instead of widening it** | `parent_kind()` is walked to termination by `depth()`, `graph-owl-cli`'s declarative validator, and the projection module's predicate namer — widening its return type to express "one of several kinds" would ripple through all three for two narrow cases. A `Container` may nest under another `Container`, and a `Column` may live directly under a `Container` for structured formats (Parquet, Avro), reusing the existing column machinery rather than inventing a new kind. Both are handled as an explicit exception in `Catalog::upsert_asset`'s containment check. Cost: the projection predicate for a nested container, or a column living under one, still names its *declared* parent kind (`parentStorageService`, `parentTable`) rather than its actual one | A third family needs the same kind of alternate parent, at which point the exception-per-call-site pattern is worth replacing with a real multi-parent signature |
| 29 | **`Storage` stays one trait at 288 methods; splitting it is deferred, not rejected** (Epic 37c Slice F, 8 August 2026) | Measured while extending `examples/embedded.rs` to a second entity family (`Topic` under `MessagingService`, Epic 34 Slice B): `awk` over the trait body in `crates/graph-owl-storage/src/lib.rs` counts 288 `fn`/`async fn` signatures across 2,901 lines. An embedder writing a *third* backend from scratch — not adopting `InMemoryStorage` or `PostgresStorage`, which already implement the whole trait and so cannot themselves demonstrate this friction — would have to implement all 288 before anything compiles, with no way to implement "just entities" first. Not split now: 288 methods sorting cleanly into `EntityStore`/`RelationshipStore`/`HistoryStore`-shaped groups is an assumption, not something this slice measured, and a live split touches every call site in `Catalog`, both adapters, and the trait definition itself — a cross-cutting breaking change disproportionate to a slice whose own plan scoped it as "small (<1 day)" | A real third backend is attempted (the friction stops being hypothetical), or a maintainer groups the 288 signatures by the entity/relationship/history boundary already implicit in `Catalog`'s own method naming and finds it holds cleanly |
