...
**Status**: In progress (Slices A-C shipped)
...
- [ ] An out-of-order event describing older state does not overwrite newer state. **Deliberately deferred to Slice D** — see Slice B's shipped note: no entity identity exists to compare "current state" against until an event is actually applied, which is Slice D's Mapped→Applied transition. The pure comparison (`compare_timestamps`) is built and tested; wiring it against real state is not.
