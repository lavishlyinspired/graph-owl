...
| SPARQL **1.2** evaluation | **The parse surface moves to 1.2 with Epic 94 Slice D, and evaluation does not follow.** Triple-term patterns need `spargebra`'s `sparql-12` feature, which is compile-time and crate-wide: it enables triple terms, reified-triple syntax, `VERSION`, `!!`, `LANGDIR`, `STRLANGDIR`, `hasLang`, `hasLangDir` in one switch. graph-owl evaluates the triple-term half and refuses whatever remains **by name**, where "whatever remains" is measured once the flag is on — `spareval` already implements `LangDir`, `StrLangDir` and `HasLang`, so the gap is narrower than the syntax list suggests. This is a deliberate acceptance of Working-Draft churn on the parse surface — see the caution above; the row is here so the acceptance is visible rather than implied | Per construct, as each is evaluated. The SPARQL 1.2 Query Language reaching Recommendation removes the churn risk but not the per-construct gap |
