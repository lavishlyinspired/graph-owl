...
| SPARQL **1.2** evaluation | **The parse surface moved to 1.2 with Epic 94 Slice B (7 August 2026), not Slice D as first planned — Cargo forced it early.** `graph-owl-rdf-io` needs `oxrdf/rdf-12` for `Term::Triple` (Slice B's own export need), and `graph-owl-rdf-io` depends on `graph-owl-query`; Cargo unifies features for one crate+version across a build graph, so `spargebra`'s and `spareval`'s own `sparql-12` had to come on in the same commit or their internal `Term`/`TermRef` matches (not this project's code) failed to compile. `sparql-12` is compile-time and crate-wide: it enables triple terms, reified-triple syntax, `VERSION`, `!!`, `LANGDIR`, `STRLANGDIR`, `hasLang`, `hasLangDir` in one switch. What Slice B actually did with the now-wider parse surface: `graph_owl_query::term::from_term` and `pushdown::named` both gained a named `Term::Triple`/`TermPattern::Triple` arm (refused, resp. treated as non-narrowing) rather than a wildcard — so the compiler still forces a decision at every site the variant reaches, just as it would have under Slice D alone. **What Slice B did not do**: the "measured list" of which SPARQL 1.2 constructs now parse-then-fail at evaluation, the `rdf:reifies` query-surface synthesis in `dataset.rs`, and the zero-rows/authorization RED tests — all still Slice D's own remaining scope. `spareval` already implements `LangDir`, `StrLangDir` and `HasLang`, so that gap is narrower than the syntax list suggests. This is a deliberate acceptance of Working-Draft churn on the parse surface — see the caution above; the row is here so the acceptance is visible rather than implied | Per construct, as each is evaluated. The SPARQL 1.2 Query Language reaching Recommendation removes the churn risk but not the per-construct gap |
