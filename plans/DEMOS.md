...
- [~] **`plan` / `apply` / `diff` with scoped authority** *(Slices A–G, 2 August 2026)*. A YAML declaration format with a purely local validator — no catalog connection, so it runs as the first step of a pull-request check — that reports **every** error rather than the first, each naming its file and line. Plan computation classifies create/update/no-change/prune with per-field before → after, and is byte-identical across runs by construction (`BTreeMap` throughout, no map iteration anywhere to reintroduce ordering noise) because a plan that reorders is undiffable in CI. Apply orders parents before children by FQN segment depth — an FQN contains its own ancestry, so no topological sort is needed — and omits unchanged entities entirely, which is what makes a second apply produce zero versions rather than merely zero *visible* change. Pruning needs both guards: a declared scope (an *undeclared* one is refused, never read as "the whole catalog") and a threshold. Exit codes separate "changes pending" from "error", so a legitimate diff is not a broken build, and plan output is redacted line-wise before it reaches a CI log.

  **The HTTP client is not built**, and the criterion above says so rather than claiming otherwise. Every module is a pure function over declarations and live state — which is why 32 tests run with no server — but nothing yet fetches live state or sends a change. Decision 6 (thin client over the API) makes that one well-defined piece to add onto a proven core.
