...
- [~] **9** JSON-LD, Turtle, DCAT, PROV-O, OpenLineage — **Slice A (Turtle, N-Triples, N-Quads) shipped, 5 August 2026; Slices B–F not started.** Checked before implementing (`verify-blockers-against-code`): the plan's stated "depends on Epic 7 (CONSTRUCT produces Turtle)" was overstated for Slice A — CONSTRUCT is not wired in `execute_algebra` today, and Slice A never needed it, since it serializes `&[Flake]` from `TripleStore::query_pattern` directly. `RdfSerializer`/`RdfParser` traits, `StandardRdfIo` wrapping `oxttl` (already-adopted, `00l-build-vs-adopt.md`), reusing the existing `graph_owl_query::term` `FlakeValue<->Term` mapping SPARQL evaluation already built rather than writing a second copy. All tested `FlakeValue` variants round-trip; blank nodes skolemized into a fresh `Sid` per unique label, stable within one parse and never reused across documents; an unregistered namespace refuses serialization with a named error, the reverse direction (an unrecognised imported IRI) refuses too; N-Quads carries `cx`, N-Triples drops it with a logged warning naming the count. JSON-LD (a served, versioned context plus SSRF-refusal testing), DCAT/PROV-O (validated against *published external* SHACL shapes, not self-written ones), and OpenLineage (a second real external spec) are each substantial, separable work — recorded as not started rather than rushed, the same explicit-stop discipline `37a-scale.md` established for its own remaining slices.
- [~] **9a** GraphML, bulk CSV, projection targets — **Slice A (streaming GraphML export) shipped, 5 August 2026; Slices B–F not started.** `GraphMlWriter` streams `<node>`/`<edge>` elements to on-disk scratch files as they arrive (never accumulated in memory), while a bounded schema map — one entry per *distinct* property key, assigned a stable id the first time it is seen — tracks what `<key>` declarations `finish()` needs to write before the `<graph>` body, then streams both scratch files straight into the final output. `quick-xml` adopted (`00l-build-vs-adopt.md`) for its auto-escaping `Writer` API. **A real bug found and fixed while writing this slice's own tests**: an early design derived each `<key>` id from its sorted position at write time, which silently disagreed with an already-written `<data key="...">` reference the moment a later element introduced a new key sorting alphabetically earlier — fixed by assigning ids on first sight and caching them, with a regression test kept specifically for this failure mode. XML escaping proven at the byte level and by feeding output through a real parser. Memory-boundedness proven structurally at 5,000 elements (schema state stays at one entry regardless of element count), not measured by OS-level RSS at the plan's own 1M-element target. `LpgReader` (import) and every other format (bulk CSV, Cypher script, JSON Graph, JSON Lines, projection to an external store) are not started — the same explicit-stop discipline `37a-scale.md`/`09-engine-rdf-io.md` already established.
