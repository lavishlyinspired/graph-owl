...
**Pending in this epic**
- **Teams do not nest.** Slice B is titled "Teams exist and nest" and specifies `parentOf` edges with cycle detection at depths 1, 2 and 3; `V13` has no parent column and none of that was built. The DEMOS line above claims only "Teams **exist**", which is honest — but Slice G's "deleting a team with child teams → `409`" depends on the nesting that does not exist, so that criterion is not reachable either
...
- [~] **Admin: the rest of the section** *(Epic 41 Slice F)* — outstanding: policy **authoring** UI, principal and group management, `SchemaForm` rendering a connector's JSON Schema with a connection test before save, and the structural test asserting `SchemaForm` is the only form renderer in admin. The dry-run and the write-only secret — the two security-critical halves — are built. Connector *runs* already have a history page. **Schedules are refused, not missing** (Epic 15 decision 5)
