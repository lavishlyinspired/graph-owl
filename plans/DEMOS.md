...
- [~] **Admin: the rest of the section** *(Epic 41 Slice F)* — the dry-run above is the security-critical half and it is built; the section around it is not. Outstanding: policy **authoring** UI, principal and group management, connector configuration via `SchemaForm` with **write-only secrets** (never rendered back, never in a response — the plan's first RED), and the structural test asserting `SchemaForm` is the only form renderer in admin. Connector *runs* already have a history page. **Schedules are refused, not missing** — Epic 15 decision 5: graph-owl does not become a scheduler
