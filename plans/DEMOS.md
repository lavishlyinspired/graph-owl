...
- [ ] **9a** GraphML, bulk CSV, projection targets — blocked on nothing from Epic 7c (shipped, and its `FlakeValue::Ref`-vs-`String` kind bug already fixed per `07d-engine-bolt.md`'s own note); needs Epic 9's streaming-serializer *shape* to mirror, which now exists as of Slice A above
