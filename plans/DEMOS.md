...
- [~] One review queue over four proposal sources — **Slices C–D shipped, 7 August 2026, 3 of 4 sources**: `ReviewQueue.tsx` proved on merge adjudication alone in Slice C (Epic 17's already-shipped resolution queue) — score, evidence, a pure `compareAssets` side-by-side diff, merge/reject/defer, reject requiring a reason, a second reviewer's `409` turned into "someone else already decided" rather than an error toast — then Slice D generalized it into a `QueueConfig` (three action shapes: instant, withText, clientOnly) and added extraction claims (Epic 21, source passage with the extracted span highlighted via a real `<mark>` — the plan's own named RED test) and drift (Epic 20, declared-vs-live diff, apply writes through to the live asset, ignore requires a reason). Proposals (Epic 35) is not built: no catalog-wide pending-proposals endpoint exists, only per-entity and per-user, and the frontend has no "who am I" endpoint either — a real backend gap, recorded rather than faked with a partial view. Live merge-splitting was also not wired (Slice C) — no endpoint maps a queue entry to its `merge_records` row yet. **Two findings from Slice C's own shipped code, surfaced only once Slice D's generalization and its own axe/Playwright checks exercised it further**: a `heading-order` violation already present, uncaught, in the vocabulary detail title (fixed in both places); and an `aria-dialog-name` violation on every antd `Modal` in this feature when scanned mid-close-animation (fixed with a settle-wait in both Slice C's and Slice D's own Playwright specs). A design bug specific to Slice D's own generalization — an unmemoized per-render config swap silently emptying a correlation map — was found and fixed the same way, by driving a real browser rather than by inspection. Full write-up, including a `workers: 1` Playwright config fix the fourth spec file exposed, in `42-ui-semantic-surfaces.md`
