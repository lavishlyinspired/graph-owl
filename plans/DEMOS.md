...
- [~] **Admin: the section exists** *(Epic 41 Slice F, 30 July 2026)*. An `Admin` nav section with two panels: **people & teams**, and **connector config rendered from the connector's own JSON Schema**. The dry-run and the write-only secret — the two security-critical halves — were already built; connector *runs* already had a history page. **Schedules are refused, not missing** (Epic 15 decision 5).
  ...
  **Outstanding**: the policy **authoring** UI, and a connection test before save. Neither is started.

**Pending in this epic**
- **Policy authoring UI** — the dry-run exists and is wired; composing a policy in the console does not
- **Connection test before save** on the connector config form. The schema renders and validates required fields, but nothing calls the connector's `test_connection` before writing a credential — so a typo is discovered by the next run rather than at the point of entry
