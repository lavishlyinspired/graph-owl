...
| **G. Filing period / per-month view** 🔶 **Slice 1 of 5 shipped** | CAs can answer "what was my position last month, what changed this month" | Already fully designed in `plans/107-filing-period.md` — pick up as written. Slice 1 (the walking skeleton — `gst:FilingPeriod` exists, one query traverses to it, commit `440c78d`) shipped; the actual per-month *view* this row promises needs Slices 2-4 (cross-period diff, subject history, console surface), none started | n/a, already scoped | See `107-filing-period.md`'s own acceptance criteria | Already release-planned; also unblocks Plan 108 Slice 6 |
