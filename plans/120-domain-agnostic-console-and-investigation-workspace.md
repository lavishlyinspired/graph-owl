...
**Status**: Slices D (`408b631`), A (`63f9dd4`), C (`plans/121-label-resolution.md`, 4 sub-slices — `af5b23e`, `954917a`, `e907755`, `bd26e60`) and E (`7f2f5d2`, narrower than originally scoped — see its own row below) shipped 16 August 2026, all verified live. B/F/G/H scoped, not started. Written in response to a direct architectural request: reframe reco-now from "reconciliation results viewer" to "domain investigation workspace powered by GraphOWL," determine what graph-owl's console's purpose actually is, move whatever is GST-specific out of the console and into reco-now, fix the data-persistence and label-resolution bugs found along the way, and restore a regression in the Ontology Builder.
