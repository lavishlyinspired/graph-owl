...
**Status**: Slice D shipped 16 August 2026 (commit `408b631`, verified live). A/B/C/E/F/G/H scoped, not started. Written in response to a direct architectural request: reframe reco-now from "reconciliation results viewer" to "domain investigation workspace powered by GraphOWL," determine what graph-owl's console's purpose actually is, move whatever is GST-specific out of the console and into reco-now, fix the data-persistence and label-resolution bugs found along the way, and restore a regression in the Ontology Builder.
