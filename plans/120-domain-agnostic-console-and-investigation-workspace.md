...
**Status**: All eight slices shipped 16 August 2026, all verified live — Plan 120 is complete. D (`408b631`), A (`63f9dd4`), C (`plans/121-label-resolution.md`, 4 sub-slices — `af5b23e`, `954917a`, `e907755`, `bd26e60`), E (`7f2f5d2`, narrower than originally scoped — see its own row below), B (`65c11ec`, restored onto the current data model rather than ported literally — see its own row below), H (`7c0d82c`, pure relocation, no behavior change — see its own row below), F (`b69c6be`, dark theme kept per direct user choice, new indigo accent — see its own row below), and G (`plans/107-done-filing-period.md`, all 5 sub-slices — `440c78d`, `df5e3bd`, `c84ae61`, `da459fc`, `eb2b37a` — see its own row below). Written in response to a direct architectural request: reframe reco-now from "reconciliation results viewer" to "domain investigation workspace powered by GraphOWL," determine what graph-owl's console's purpose actually is, move whatever is GST-specific out of the console and into reco-now, fix the data-persistence and label-resolution bugs found along the way, and restore a regression in the Ontology Builder.

**Companions**: `plans/00a-product-position.md` (graph-owl's stated purpose — read before anything else here), `plans/00f-ui-architecture.md` / `00h-ui-design-system.md` (console patterns and budgets), `plans/105f-gst-console-surfaces.md` (an already-existing gap analysis this plan builds directly on), `plans/107-done-filing-period.md` (the per-month/filing-period epic — already scoped, unbuilt, not re-scoped here), `plans/119-architecture-audit.md` (the reco-now/graph-owl integration work this plan follows on from).
...
`plans/105f-gst-console-surfaces.md` §1 (11 August 2026) already names this "the biggest gap," and `plans/107-done-filing-period.md` already designs `gst:FilingPeriod` as a first-class graph entity, plus the query and slice sequencing to answer "what's outstanding for period X" and "show this invoice across every period it's appeared in." `plans/108-books-gstr1-gstr2b-reconciliation.md`'s own status line said, at the time this plan was written, its Slice 6 (period-aware carry-forward) *"remains blocked on `107-filing-period.md` Slice 1, still unbuilt"* — quoted here verbatim as it stood then; both plans have since shipped and the quote is superseded history, not a current claim.
...
| **G. Filing period / per-month view** ✅ **Shipped, all 5 sub-slices** | CAs can answer "what was my position last month, what changed this month" | `plans/107-done-filing-period.md`'s full 5-slice plan, all shipped: `gst:FilingPeriod` as a graph entity (1, `440c78d`), `period-diff` (2, `df5e3bd`), `period-history` (3, `c84ae61`), the console surface at `ui/src/features/filingPeriods` (4, `da459fc`), `periods-before` for "this month vs. last" (5, `eb2b37a`) | Calendar-locale reasoning, fiscal-year handling — see `107-done-filing-period.md`'s own Defers per slice | See `107-done-filing-period.md`'s own acceptance criteria, all verified live | Shipped; also unblocks Plan 108 Slice 6 |
