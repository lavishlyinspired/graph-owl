...
**`deny.toml` and the CI job were owed since this section was written, not created until Epic 103's own stated prerequisite forced the question, 8 August 2026.** Wiring it up immediately found four real findings on already-shipped dependencies — proof this was overdue, not a formality:

- **`option-ext` (MPL-2.0), transitive via `rust-embed`** (UI embedding). Weak, file-level copyleft — the same class this document already rejected for `elkjs`/EPL-2.0. Added as a **narrow, per-crate `deny.toml` exception**, not a blanket MPL-2.0 allowance, and left open rather than resolved: `rust-embed` is the current, actively-maintained, no-alternative way this project embeds the built console, and ripping it out was a bigger, riskier decision than this pass had grounds to make unilaterally. Revisit: find a permissive alternative, or make a deliberate, recorded decision that an unmodified (never forked, never patched) MPL-2.0 dependency's file-level trigger does not reach this project's own code and is acceptable as read.
- **`webpki-roots` (CDLA-Permissive-2.0), transitive via `reqwest`/`rustls`**. Added to the **main allowlist**, not an exception — CDLA-Permissive-2.0 is a data licence (root-certificate bundles, not code) with no copyleft trigger under any circumstance, and `webpki-roots` is the de facto standard for the entire Rust rustls ecosystem (650M downloads). Absent from the original enumerated list only because root-cert data never came up before this gate actually ran.
- **RUSTSEC-2026-0194 and -0195 (quick-xml, quadratic parsing / unbounded namespace allocation, both DoS)**, reachable via `sparesults`/`spareval` — this project's own SPARQL federation `SERVICE` clause parses XML results returned by a remote endpoint, the exact reachability shape both advisories cite as exploited in the wild elsewhere. A fix exists (`quick-xml >= 0.41`, already what the rest of this workspace resolves to) but `sparesults` 0.3.3 — the latest release — hard-pins `quick-xml = "^0.37"`; `[patch.crates-io]` cannot override a dependency's own declared compatible range without forking it. Accepted via `deny.toml`'s `advisories.ignore`, flagged prominently rather than silently passed, pending an upstream `sparesults` release.
- **RUSTSEC-2025-0111 (tokio-tar) and RUSTSEC-2025-0134 (rustls-pemfile, unmaintained)**, both dev-only via `testcontainers`/`bollard` — never reach the production binary. Accepted the same way, lower urgency.

See `deny.toml` itself for the exact entries and reasoning; this is the summary, not the source of truth.
