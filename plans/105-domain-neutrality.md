...
- [ ] **1.7** `namespaces` registry table (`V14`) + storage adapter load, mirroring the `predicates` table's `core` protection
...
- [ ] **2.1** RED: a strategy named after an algorithm produces the same key for two records a domain calls equal, and a different key for two it does not
- [ ] **2.2** `exact`, `normalized`, `phonetic`, `ngram`, `numeric_bucket`, `date_window`, `composite` over caller-named predicates
- [ ] **2.3** The existing four catalog keys expressed as configurations of the generic set (proof the generalization is real, not additive)
- [ ] **2.4** Mutation run; survivors triaged
