  ...
  return (
    <header className="flex h-[52px] flex-none items-center gap-5 border-b border-gowl-line bg-gowl-panel px-4.5">
      <div className="flex items-center gap-2.5">
        ...
        <span className="text-[17px] font-semibold tracking-[0.14em]">{strings.brand}</span>
      </div>
      ...
      <button
        ...
        className="rounded-md border border-gowl-line px-2.5 py-1 font-mono text-[14px] tracking-widest text-gowl-t5 hover:border-gowl-line-2"
      >
        {theme === "dark" ? strings.themeNight : strings.themeDay}
      </button>

      <div className="font-mono text-[15px] text-gowl-t5">{asOf}</div>
      ...
      >
        ...
        {pendingCount > 0 && (
          <span className="ml-1.5 rounded-full bg-gowl-amber-bg px-1.5 py-0.5 font-mono text-[14px] text-gowl-amber">
            {pendingCount}
          </span>
        )}
      </button>

      <div className="flex h-7 w-7 items-center justify-center rounded-full bg-gowl-avatar font-mono text-[15px] text-gowl-t2">
        {userInitials}
      </div>
    </header>
  );
