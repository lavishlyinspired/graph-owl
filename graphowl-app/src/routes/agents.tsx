      ...
      <div className="mb-6 grid grid-cols-5 gap-px overflow-hidden rounded-lg border border-gowl-line bg-gowl-line">
        {KPI_STATS.map((stat) => (
          <div key={stat.label} className="bg-gowl-panel p-4">
            <div className="mb-2 font-mono text-[9.5px] tracking-widest text-gowl-t6">{stat.label}</div>
            <div className={`font-mono text-[20px] ${stat.color ?? "text-gowl-t1"}`}>{stat.value}</div>
            <div className="mt-1 text-[11px] text-gowl-t7">{stat.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-[1fr_400px] gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg border border-gowl-line bg-gowl-panel p-5">
            ...
            <p className="mb-4 text-[11.5px] text-gowl-t5">{strings.agentsPipelineSubtitle}</p>
              ...
              {PIPELINE_STAGES.map((stage, i) => (
                <div key={stage.tag} className="flex items-center gap-2">
                  <div className={`flex-none rounded-lg border ${stage.borderColor} ${stage.bgColor} p-3`}>
                    <div className="mb-1 font-mono text-[8px] tracking-widest text-gowl-t6">{stage.tag}</div>
                    <div className="text-[12px] font-semibold text-gowl-t1">{stage.name}</div>
                    <div className="mt-0.5 text-[10px] text-gowl-t5">{stage.detail}</div>
                  </div>
                  {i < PIPELINE_STAGES.length - 1 && (
                    <span className="flex-none font-mono text-[14px] text-gowl-t6">→</span>
                  )}
                </div>
              ))}
            </div>
            <p className="mt-3 text-[10.5px] leading-relaxed text-gowl-t5">
          ...
          <div className="rounded-lg border border-gowl-line bg-gowl-panel p-5">
            ...
            <p className="mb-4 text-[11px] text-gowl-t5">{strings.agentsRosterSubtitle}</p>
            <div className="overflow-hidden rounded-md border border-gowl-line-2">
              <div className="grid grid-cols-[1.5fr_150px_118px_92px_150px] gap-2 border-b border-gowl-line bg-gowl-panel-2 px-3 py-2 font-mono text-[8.5px] tracking-wider text-gowl-t6">
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[12.5px] text-gowl-t1">{agent.name}</span>
                        <span className={`rounded-full px-1.5 py-0.5 font-mono text-[8px] ${ms.bg} ${ms.text}`}>
                          {agent.mode}
                        </span>
                      </div>
                      <div className="mt-0.5 text-[10.5px] text-gowl-t5">{agent.desc}</div>
                    </div>
                    <div>
                      <div className="text-[11.5px] text-gowl-t2">{agent.trigger}</div>
                      <div className="font-mono text-[10px] text-gowl-t5">{agent.next}</div>
                    </div>
                    <div>
                      <div className="font-mono text-[12px] text-gowl-t1">{agent.runs}</div>
                      <div className="font-mono text-[10px] text-gowl-t5">{agent.tokens} tokens</div>
                    </div>
                    <div className={`font-mono text-[12px] ${parseFloat(agent.grounding) > 95 ? "text-gowl-ok" : "text-gowl-amber"}`}>
                      {agent.grounding}
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <button type="button" className="text-[11px] text-gowl-accent">
                        Run now
                      </button>
                    </div>
                  </div>
          ...
          {/* Run Trace */}
          <div className="rounded-lg border border-gowl-line bg-gowl-panel p-5">
            <div className="mb-1 font-mono text-[9.5px] tracking-widest text-gowl-t6">
              {strings.agentsTraceTitle}
            </div>
            <div className="mb-3 font-mono text-[10px] text-gowl-t5">
              {strings.agentsTraceMeta}
            </div>
            <p className="mb-4 text-[12.5px] text-gowl-t2">
            ...
            <div className="space-y-2">
              {TRACE_STEPS.map((step, i) => (
                <div key={i} className="flex items-center gap-3 rounded-md border border-gowl-line-2 bg-gowl-panel-2 px-3 py-2">
                  <span className="flex-none font-mono text-[10px] text-gowl-t6">{i + 1}</span>
                  <span className="flex-none font-mono text-[11px] text-gowl-accent">{step.tool}</span>
                  <span className="flex-1 truncate font-mono text-[10.5px] text-gowl-t4">{step.args}</span>
                  <span className="flex-none font-mono text-[10px] text-gowl-t5">{step.tokens} tok</span>
                  <span className="flex-none font-mono text-[10px] text-gowl-t5">{step.latency}</span>
                </div>
              ))}
            </div>
            <div className="mt-4 rounded-md border border-gowl-accent-border bg-gowl-accent-deep p-3">
              ...
              <p className="text-[12px] leading-relaxed text-gowl-t2">
                {strings.agentsAnswerBody}
              </p>
              <div className="mt-2 text-[10.5px] text-gowl-t5">{strings.agentsAnswerCite}</div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-3">
              {GUARDRAILS.map((item, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <span className={`mt-0.5 flex-none text-[14px] ${item.ok ? "text-gowl-ok" : "text-gowl-amber"}`}>
                    {item.ok ? "✓" : "!"}
                  </span>
                  <div>
                    <div className="text-[12px] text-gowl-t2">{item.text}</div>
                    <div className="mt-0.5 text-[10.5px] text-gowl-t5">{item.meta}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-lg border border-gowl-line bg-gowl-panel p-5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {MCP_TOOLS.map((tool) => (
                <span
                  key={tool}
                  className="rounded border border-gowl-line-2 bg-gowl-input px-2 py-1 font-mono text-[10px] text-gowl-t4"
                >
                  {tool}
                </span>
              ))}
