  ...
  return (
    <div className="flex h-full">
      <div className="flex-1 overflow-y-auto p-8">
        <h1 className="mb-1 text-[21px] font-semibold text-gowl-t1">{strings.driftTitle}</h1>
        <p className="mb-5 text-[12.5px] text-gowl-t5">{strings.driftDescription}</p>
        ...
        <div className="overflow-hidden rounded-lg border border-gowl-line bg-gowl-panel">
          ...
          {pending.length === 0 ? (
            <div className="p-6 text-[12.5px] text-gowl-t5">{strings.driftEmpty}</div>
          ) : (
            pending.map((item) => (
              <button
                key={item.id}
              ...
              >
                <span className="truncate text-[12.5px] text-gowl-t1">{item.field}</span>
                <span className="truncate font-mono text-[11.5px] text-gowl-t2">{item.liveValue ?? "—"}</span>
                <span className="truncate font-mono text-[11.5px] text-gowl-t2">{item.declaredValue ?? "—"}</span>
                <span className="text-[12px] text-gowl-t5">{item.kind}</span>
                <span className="truncate text-[11px] text-gowl-t6">{item.fullyQualifiedName}</span>
              </button>
            ))
          )}
        </div>
      ...
      {selected && (
        <div className="w-[380px] flex-none overflow-y-auto border-l border-gowl-line bg-gowl-panel p-5">
          <div className="mb-4 flex items-start justify-between">
            <div>
              <div className="font-mono text-[11px] text-gowl-t6">{selected.fullyQualifiedName}</div>
              <div className="text-[15px] font-semibold text-gowl-t1">{selected.field}</div>
            </div>
          ...
          {canApply(selected) ? (
            ...
            <p className="mb-2 text-[11.5px] text-gowl-t6">{strings.driftApplyUnavailable}</p>
          )}
          <input
            value={ignoreReason}
