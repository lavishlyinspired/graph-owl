    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex-none border-b border-gowl-line bg-gowl-panel px-6 py-4">
        <div className="mb-4 flex items-end justify-between">
          <div>
            <h1 className="mb-1 text-[24px] font-semibold text-gowl-t1">{strings.pipelineTitle}</h1>
            <p className="text-[16.5px] text-gowl-t5">{strings.pipelineSubtitle}</p>
          </div>
            ...
            <div key={step.key} className="flex flex-1 items-start">
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center gap-2">
                  <span className={`flex h-[18px] w-[18px] flex-none items-center justify-center rounded-full border text-[13px] ${
                    step.done
                  ...
                  <span className={`whitespace-nowrap text-[16.5px] font-medium ${
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="w-[236px] flex-none border-r border-gowl-line bg-gowl-panel p-3">
          <div className="mb-2.5 px-2 font-mono text-[13px] tracking-widest text-gowl-t7">{strings.pipelineDatasetsTitle}</div>
          {DATASETS.map((d, i) => (
            <button
              key={d.name}
              type="button"
            ...
            >
              <div className="flex items-center gap-2">
                <span className={`text-[16.5px] ${selectedDataset === i ? "text-gowl-accent" : "text-gowl-t3"}`}>{d.name}</span>
                <span className={`ml-auto text-[14px] ${d.mapped === d.total ? "text-gowl-ok" : "text-gowl-amber"}`}>
                  {d.mapped}/{d.total}
                </span>
              </div>
              <div className="mt-0.5 font-mono text-[13.5px] text-gowl-t7">{d.meta}</div>
            </button>
          ))}
          <div className="mt-3.5 border-t border-gowl-line px-1 pt-3.5 text-[15.5px] leading-relaxed text-gowl-t6">
        ...
        <div className="flex-1 min-w-0 overflow-auto p-5">
          ...
          <div className="mb-3.5 flex items-center gap-3">
            <span className="text-[18px] font-semibold text-gowl-t1">{ds.name}</span>
            <span className="font-mono text-[14.5px] text-gowl-t6">{ds.meta}</span>
            <button type="button" className="ml-auto rounded border border-gowl-accent-border bg-gowl-accent-bg px-2.5 py-1 text-[15.5px] text-gowl-accent">
          ...
          <div className="mb-3.5 overflow-hidden rounded-lg border border-gowl-line bg-gowl-panel">
            <div className="grid grid-cols-[1.5fr_1.2fr_1.2fr_96px] gap-3 border-b border-gowl-line bg-gowl-panel-2 px-4 py-2.5 font-mono text-[13.5px] tracking-wider text-gowl-t6">
            ...
            {MAPPINGS.map((m) => (
              ...
              >
                <div>
                  <div className="font-mono text-[15.5px] text-gowl-accent">
                    {m.property}{m.required && <span className="text-gowl-bad">*</span>}
                  </div>
                  <div className="mt-0.5 font-mono text-[13.5px] text-gowl-t7">{m.className}</div>
                </div>
                <div className="flex items-center gap-2 rounded border border-gowl-line-2 px-2 py-1">
                  <span className="font-mono text-[15px] text-gowl-t3">{m.sourceColumn}</span>
                  <span className="ml-auto text-[14px] text-gowl-t7">▾</span>
                </div>
                <span className="font-mono text-[15px] text-gowl-t3">{m.sample}</span>
                <span className={`font-mono text-[14.5px] text-right ${
            ...
            <div className="flex items-center justify-between px-4 py-2.5">
              <span className="text-[15.5px] text-gowl-t6">{MAPPINGS.length} mappings</span>
              <button type="button" className="rounded-md border border-gowl-accent-border bg-gowl-accent-bg px-3 py-1.5 text-[16px] text-gowl-accent">
          ...
          <div className="grid grid-cols-2 gap-3.5">
            <div className="rounded-lg border border-gowl-line bg-gowl-panel p-4">
              <div className="mb-2.5 font-mono text-[13.5px] tracking-widest text-gowl-t6">{strings.pipelineUnmappedTitle}</div>
              <div className="mb-2.5 flex flex-wrap gap-1.5">
                {["created_at", "updated_by", "internal_code"].map((col) => (
                  <span key={col} className="rounded bg-gowl-input border border-gowl-line-2 px-1.5 py-0.5 font-mono text-[14px] text-gowl-t5">
                    {col}
                  </span>
                ))}
              </div>
              <div className="text-[15.5px] leading-relaxed text-gowl-t6">{strings.pipelineUnmappedNote}</div>
            </div>
            <div className="rounded-lg border border-gowl-line bg-gowl-panel p-4">
              <div className="mb-2.5 font-mono text-[13.5px] tracking-widest text-gowl-t6">{strings.pipelineWhatDecidesTitle}</div>
              <div className="text-[16px] leading-relaxed text-gowl-t4">{strings.pipelineWhatDecidesNote}</div>
            </div>
          </div>
        </div>
      </div>
