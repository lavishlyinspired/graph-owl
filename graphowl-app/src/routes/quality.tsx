...
export default function QualityRoute() {
  return (
    <div className="p-8">
      <div className="mb-5">
        <h1 className="mb-1 text-[21px] font-semibold text-gowl-t1">{strings.qualityTitle}</h1>
        <p className="text-[12.5px] text-gowl-t5">{strings.qualityDescription}</p>
      </div>
        ...
        {METRICS.map((metric) => (
          <div key={metric.name} className="rounded-lg border border-gowl-line bg-gowl-panel p-5">
            <div className="mb-2 flex items-baseline justify-between">
              <span className="text-[13px] font-semibold text-gowl-t1">{metric.name}</span>
              <div className="flex items-baseline gap-3">
                <span className="font-mono text-[12px] text-gowl-t5">{metric.trend}</span>
                <span className="font-mono text-[18px] text-gowl-t1">{metric.score}</span>
              </div>
            </div>
            ...
            <p className="text-[11.5px] text-gowl-t5">{metric.description}</p>
          </div>
        ))}
      </div>
    </div>
