  ...
  return (
    <div className="p-8">
      <div className="mb-5 flex items-end justify-between">
        <div>
          <h1 className="mb-1 text-[25px] font-semibold text-gowl-t1">{strings.tasksTitle}</h1>
          <p className="text-[16.5px] text-gowl-t5">{strings.tasksDescription}</p>
        </div>
        <div className="text-[16px] text-gowl-t5">{openCount} open tasks</div>
      </div>

      <div className="overflow-hidden rounded-lg border border-gowl-line">
        <div className="grid grid-cols-[1fr_120px_90px_90px_80px_110px] gap-2 border-b border-gowl-line bg-gowl-panel-2 px-4 py-2.5 font-mono text-[12.5px] tracking-wider text-gowl-t6">
          <span>TASK</span>
          <span>ASSIGNEE</span>
          <span>PRIORITY</span>
          <span>STATUS</span>
        ...
        {MOCK_TASKS.map((task) => {
          ...
          return (
            ...
            >
              <span className="text-[16.5px] text-gowl-t1">{task.title}</span>
              <span className="text-[15.5px] text-gowl-t4">{task.assignee}</span>
              <span className={`rounded-full px-2 py-0.5 text-center font-mono text-[12.5px] ${ps.bg} ${ps.text}`}>
                {task.priority.toUpperCase()}
              </span>
              <span className={`rounded-full px-2 py-0.5 text-center font-mono text-[12.5px] ${ss.bg} ${ss.text}`}>
                {task.status.toUpperCase()}
              </span>
              <span className="font-mono text-[15px] text-gowl-t5">{task.due}</span>
              <span className="text-[14.5px] text-gowl-t5">{task.source}</span>
            </div>
          );
        })}
      </div>
