  ...
  return (
    <div className="p-8">
      <div className="mb-5 flex items-end justify-between">
        <div>
          <h1 className="mb-1 text-[22.5px] font-semibold text-gowl-t1">{config.title}</h1>
          <p className="text-[14px] text-gowl-t5">{config.description}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-[1fr_300px] items-start gap-4">
        <div className="overflow-hidden rounded-lg border border-gowl-line bg-gowl-panel">
          <div
            className="grid gap-3 border-b border-gowl-line bg-gowl-panel-2 px-4 py-2 font-mono text-[11px] tracking-wider text-gowl-t6"
          ...
          {config.rows.length === 0 ? (
            <div className="p-6 text-[14px] text-gowl-t5">{config.emptyMessage}</div>
          ) : (
            config.rows.map((row) => (
              <div
                key={row.key}
              ...
              >
                {row.cells.map((cell, index) => (
                  <div key={index}>
                    <div className={cell.mono ? "truncate font-mono text-[13px] text-gowl-t2" : "truncate text-[14px] text-gowl-t1"}>
                      {cell.text}
                    </div>
                    {cell.sub && <div className="mt-0.5 truncate font-mono text-[11.5px] text-gowl-t7">{cell.sub}</div>}
                  </div>
                ))}
              </div>
            ))
        ...
        <div className="flex flex-col gap-3.5">
          <div className="rounded-lg border border-gowl-line bg-gowl-panel p-4">
            <div className="mb-2.5 font-mono text-[11px] tracking-widest text-gowl-t6">{config.noteTitle}</div>
            <div className="text-[14px] leading-relaxed text-gowl-t3">{config.noteBody}</div>
          </div>
          <div className="rounded-lg border border-gowl-line bg-gowl-panel p-4">
            <div className="mb-2.5 font-mono text-[11px] tracking-widest text-gowl-t6">{strings.relatedTitle}</div>
...
function RelatedLink({ label, route, id }: { readonly label: string; readonly route: string; readonly id?: string }) {
  ...
  return (
    <Link
      to={to}
      className="flex items-center justify-between border-b border-gowl-row py-1.5 text-[13.5px] text-gowl-accent last:border-b-0"
    >
      <span>{label}</span>
    </Link>
  );
