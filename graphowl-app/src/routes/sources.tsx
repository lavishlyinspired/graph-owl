  ...
  return (
    <div className="flex h-full">
      <div className="flex-1 overflow-y-auto p-8">
        <h1 className="mb-1 text-[21px] font-semibold text-gowl-t1">{strings.sourcesTitle}</h1>
        <p className="mb-5 text-[12.5px] text-gowl-t5">{strings.sourcesDescription}</p>
        ...
        <div className="overflow-hidden rounded-lg border border-gowl-line bg-gowl-panel">
          ...
          {sources.length === 0 ? (
            <div className="p-6 text-[12.5px] text-gowl-t5">{strings.sourcesEmpty}</div>
          ) : (
            sources.map((source) => (
              <button
              ...
              >
                <span className="truncate text-[12.5px] text-gowl-t1">{source.serviceName}</span>
                <span className="font-mono text-[11.5px] text-gowl-t2">{source.connector}</span>
                <span className="font-mono text-[12px] text-gowl-t1">{source.objects.toLocaleString()}</span>
                <span className="text-[12px] text-gowl-t5">{relativeTime(source.lastSyncAt, now)}</span>
                <span className={`text-[12px] font-semibold ${HEALTH_COLOR[source.health]}`}>
      ...
      {selected && (
        <div className="w-[420px] flex-none overflow-y-auto border-l border-gowl-line bg-gowl-panel p-5">
          <div className="mb-4 flex items-start justify-between">
            <div>
              <div className="font-mono text-[11px] text-gowl-t6">{selected.connector}</div>
              <div className="text-[15px] font-semibold text-gowl-t1">{selected.serviceName}</div>
            </div>
          ...
          <div className="mb-4 rounded-md border border-gowl-line-2 bg-gowl-panel-2 p-3">
            <div className="mb-2 font-mono text-[9.5px] tracking-widest text-gowl-t6">{strings.sourcesNoteTitle}</div>
            <div className="text-[12px] leading-relaxed text-gowl-t3">{strings.sourcesNoteBody}</div>
          </div>

          <div className="mb-2 font-mono text-[9.5px] tracking-widest text-gowl-t6">{strings.sourcesRunHistory}</div>
          <div className="overflow-hidden rounded-md border border-gowl-line-2">
            {historyFor(selected.serviceName).map((run) => (
              <div key={run.id} className="border-b border-gowl-row px-3 py-2 text-[11.5px] last:border-b-0">
                ...
                <div className="mt-0.5 font-mono text-[10.5px] text-gowl-t5">
