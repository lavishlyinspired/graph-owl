    ...
    <div className="flex h-full">
      <div className="flex-1 overflow-y-auto p-8">
        <div className="mb-5 flex items-end justify-between">
          <div>
            <h1 className="mb-1 text-[25px] font-semibold text-gowl-t1">{strings.adminTitle}</h1>
            <p className="text-[16.5px] text-gowl-t5">{strings.adminDescription}</p>
          </div>
        ...
        <div className="overflow-hidden rounded-lg border border-gowl-line bg-gowl-panel">
          <div className="grid grid-cols-[120px_1.2fr_1.2fr_130px_96px] gap-3 border-b border-gowl-line bg-gowl-panel-2 px-4 py-2 font-mono text-[13.5px] tracking-wider text-gowl-t6">
          ...
          {AUDIT_EVENTS.map((event, index) => (
            ...
            >
              <span className="truncate text-[16.5px] text-gowl-t1">{event.actor}</span>
              <span className="truncate text-[16.5px] text-gowl-t2">{event.action}</span>
              <span className="truncate font-mono text-[15.5px] text-gowl-t2">{event.object}</span>
              <span className="font-mono text-[15.5px] text-gowl-t5">{event.time}</span>
              <span
      ...
      <div className="w-[280px] flex-none overflow-y-auto border-l border-gowl-line bg-gowl-panel p-5">
        <div className="mb-5">
          ...
          <p className="mb-3 text-[15px] text-gowl-t5">{strings.adminChartSubtitle}</p>
          <div className="flex items-end gap-1.5" style={{ height: 100 }}>
            {AUDIT_HOURLY.map((bar) => (
              <div key={bar.hour} className="flex flex-1 flex-col items-center gap-1">
                ...
                />
                <span className="font-mono text-[13px] text-gowl-t6">{bar.hour}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mb-5 rounded-md border border-gowl-accent-border bg-gowl-accent-deep p-3">
          ...
          <p className="text-[15.5px] leading-relaxed text-gowl-t3">
          ...
          <div className="space-y-1">
            {[
              ...
              <a
                key={link.route}
                href={`/${link.route}`}
                className="flex items-center justify-between rounded-md px-2.5 py-1.5 text-[16.5px] text-gowl-t2 hover:bg-gowl-row"
              >
                <span>{link.label}</span>
                <span className="font-mono text-[14px] text-gowl-t5">{link.detail}</span>
              </a>
            ))}
          </div>
        </div>
