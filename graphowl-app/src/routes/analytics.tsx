  ...
  return (
    <div className="overflow-y-auto p-8">
      <div className="mb-5 flex items-end justify-between">
        <div>
          <h1 className="mb-1 text-[25px] font-semibold text-gowl-t1">{strings.analyticsTitle}</h1>
          <p className="text-[16.5px] text-gowl-t5">{strings.analyticsDescription}</p>
        </div>
        ...
        {KPI_CARDS.map((kpi) => (
          <div key={kpi.label} className="rounded-lg border border-gowl-line bg-gowl-panel p-4">
            ...
            <div className="flex items-baseline gap-2">
              <span className="font-mono text-[25px] text-gowl-t1">{kpi.value}</span>
              <span className="font-mono text-[15.5px] text-gowl-ok">{kpi.delta}</span>
            </div>
            ...
            <div className="text-[15px] text-gowl-t7">{kpi.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-6">
        ...
        <div className="rounded-lg border border-gowl-line bg-gowl-panel p-5">
          ...
          <div className="mb-3 flex justify-between font-mono text-[14px] text-gowl-t6">
            {GROWTH_DATA.map((col) => (
          ...
          <div className="mb-3 flex gap-4">
            {GROWTH_LEGEND.map((item) => (
              <div key={item.label} className="flex items-center gap-1.5 text-[15px] text-gowl-t4">
                <div className="h-2 w-2 rounded-sm" style={{ backgroundColor: item.color }} />
                {item.label}
              </div>
            ))}
          </div>
          <p className="text-[15.5px] leading-relaxed text-gowl-t5">
        ...
        <div className="rounded-lg border border-gowl-line bg-gowl-panel p-5">
            ...
            {FUNNEL_DATA.map((step) => (
              <div key={step.label}>
                <div className="mb-1 flex items-baseline justify-between">
                  <span className="text-[16px] text-gowl-t2">{step.label}</span>
                  <span className="font-mono text-[15px] text-gowl-t5">{step.value}</span>
                </div>
                ...
                <div className="mt-0.5 flex justify-between text-[14px] text-gowl-t6">
                  <span>{step.pct}</span>
                  <span>{step.note}</span>
                </div>
              </div>
          ...
          <p className="mt-4 text-[15.5px] leading-relaxed text-gowl-t5">
      ...
      <div className="grid grid-cols-[1fr_380px] gap-6">
          ...
          <div className="rounded-lg border border-gowl-line bg-gowl-panel p-5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-[15px]">
                <thead>
                  <tr>
                    <th className="pb-2 pr-3 text-left font-mono text-[13.5px] tracking-wider text-gowl-t6">
                      PREDICATE
                    </th>
                    {HEATMAP_MONTHS.map((month) => (
                      <th key={month} className="pb-2 px-2 text-center font-mono text-[13.5px] tracking-wider text-gowl-t6">
                        {month}
                      </th>
                    ))}
                  </tr>
                    ...
                    <tr key={row.predicate}>
                      ...
                      {row.values.map((val, i) => (
                        <td key={i} className="px-2 py-1">
                          <div
                            className={`flex h-8 items-center justify-center rounded-md font-mono text-[14px] ${HEATMAP_COLORS[row.levels[i] ?? 0]}`}
                          >
                            {val}
                          </div>
                        </td>
          ...
          <div className="rounded-lg border border-gowl-line bg-gowl-panel p-5">
            ...
            <div className="space-y-2.5">
              {HOTSPOTS.map((item) => (
                <div key={item.label}>
                  <div className="mb-1 flex items-baseline justify-between">
                    <span className="font-mono text-[16px] text-gowl-t2">{item.label}</span>
                    <span className="font-mono text-[15px] text-gowl-t5">{item.value}</span>
                  </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-gowl-accent-border bg-gowl-accent-deep p-5">
            <div className="mb-2 font-mono text-[13.5px] tracking-widest text-gowl-accent">
              {strings.analyticsNarrativeTitle}
            </div>
            <p className="mb-3 text-[16.5px] leading-relaxed text-gowl-t2">
              {strings.analyticsNarrativeBody}
            </p>
            <div className="text-[15px] text-gowl-t5">{strings.analyticsNarrativeCite}</div>
          </div>
