  ...
  return (
    <div className="p-8">
      <div className="mb-5 flex items-end justify-between">
        <div>
          <h1 className="mb-1 text-[25px] font-semibold text-gowl-t1">{strings.securityTitle}</h1>
          <p className="text-[16.5px] text-gowl-t5">{strings.securityDescription}</p>
        </div>
      </div>
      ...
      <div className="mb-6 grid grid-cols-4 gap-px overflow-hidden rounded-lg border border-gowl-line bg-gowl-line">
        {[
          ...
          <div key={kpi.label} className="bg-gowl-panel p-4">
            <div className="mb-2 font-mono text-[13px] tracking-widest text-gowl-t6">{kpi.label}</div>
            <div className={`font-mono text-[24px] ${kpi.color}`}>{kpi.value}</div>
          </div>
        ))}
      </div>
        ...
        {EVENTS.map((event) => {
          ...
          return (
            <div key={event.id} className="flex items-center gap-4 border-b border-gowl-row px-4 py-3 last:border-b-0">
              ...
              <span className="flex-none w-[130px] text-[15.5px] text-gowl-t4">{event.type}</span>
              <span className="flex-none w-[180px] truncate text-[15.5px] text-gowl-t2">{event.actor}</span>
              <span className="flex-1 text-[16px] text-gowl-t1">{event.detail}</span>
              <span className="flex-none text-[15px] text-gowl-t5">{event.time}</span>
            </div>
          );
        })}
      </div>
