    ...
    <div>
      <div className="mb-6 grid grid-cols-5 gap-3">
        {EXPORT_FORMATS.map((fmt) => (
          ...
          >
            <div className="mb-1 text-[22px]">{fmt.icon}</div>
            <div className="text-[16.5px] font-semibold text-gowl-t1">{fmt.name}</div>
            <div className="mt-0.5 text-[14.5px] text-gowl-t5">{fmt.description}</div>
          </button>
        ))}
      </div>
      ...
      <div className="mb-6 flex items-center gap-3">
        <button
          type="button"
          onClick={startExport}
          className="rounded-md bg-gowl-accent px-5 py-2 text-[16.5px] font-semibold text-gowl-accent-on"
        >
          Export as {EXPORT_FORMATS.find((f) => f.id === selected)?.name}
        </button>
        <span className="text-[15.5px] text-gowl-t5">
      ...
      <div className="rounded-md border border-gowl-line bg-gowl-panel">
        <div className="border-b border-gowl-line px-4 py-2.5 font-mono text-[13px] tracking-widest text-gowl-t6">
          EXPORT HISTORY
        </div>
        {jobs.length === 0 ? (
          <div className="p-8 text-center text-[16.5px] text-gowl-t5">No exports yet.</div>
        ) : (
              ...
              return (
                <div key={job.id} className="flex items-center justify-between px-4 py-3">
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-[15px] text-gowl-t4">{job.format}</span>
                    <span className={`rounded-full px-2 py-0.5 font-mono text-[12.5px] ${ss.bg} ${ss.text}`}>
                      {job.status.toUpperCase()}
                    </span>
                  </div>
                  <div className="flex items-center gap-4 text-[15px] text-gowl-t5">
