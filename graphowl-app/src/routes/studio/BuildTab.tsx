  ...
  return (
    <div>
      <button
        ...
        className={`block w-full truncate py-1 text-left text-[12.5px] hover:text-gowl-accent ${
  ...
  return (
    <div className="grid grid-cols-[280px_1fr] gap-4">
      <div className="rounded-lg border border-gowl-line bg-gowl-panel p-3">
        <div className="mb-2 flex gap-1">
          <input
            ...
            className="flex-1 rounded-md border border-gowl-line-2 bg-gowl-input px-2 py-1 text-[12px] text-gowl-t1"
          />
          <button
            type="button"
        ...
        {tree && tree.roots.length === 0 ? (
          <p className="text-[12px] text-gowl-t5">{strings.buildTreeEmpty}</p>
        ) : (
          tree?.roots.map((root) => (
      ...
      <div className="rounded-lg border border-gowl-line bg-gowl-panel p-5">
        {!selected ? (
          <p className="text-[13px] text-gowl-t5">{strings.buildDetailPlaceholder}</p>
        ) : (
          <div>
            <div className="mb-3 flex items-start justify-between">
              <div className="text-[16px] font-semibold text-gowl-t1">{selected.name}</div>
              <button type="button" disabled={busy} onClick={runDelete} className="text-[12px] text-gowl-bad">
                {strings.buildDeleteTerm}
              </button>
            </div>
            {selectedNode?.isCyclic && (
              <p className="mb-3 rounded-md border border-gowl-amber-border bg-gowl-amber-bg p-2 text-[12px] text-gowl-amber">
                {strings.buildCyclicNotice}
              </p>
            )}

            <div className="mb-3">
              <div className="mb-1 font-mono text-[9.5px] tracking-widest text-gowl-t6">{strings.buildDefinition}</div>
              <p className="text-[12.5px] text-gowl-t2">{selected.definition || "—"}</p>
            </div>
            <div className="mb-3 grid grid-cols-3 gap-3">
              <div>
                <div className="mb-1 font-mono text-[9.5px] tracking-widest text-gowl-t6">{strings.buildStatus}</div>
                <p className="text-[12.5px] text-gowl-t2">{selected.status}</p>
              </div>
              <div>
                <div className="mb-1 font-mono text-[9.5px] tracking-widest text-gowl-t6">{strings.buildSynonyms}</div>
                <p className="text-[12.5px] text-gowl-t2">{selected.synonyms.join(", ") || "—"}</p>
              </div>
              <div>
                <div className="mb-1 font-mono text-[9.5px] tracking-widest text-gowl-t6">{strings.buildAbbreviations}</div>
                <p className="text-[12.5px] text-gowl-t2">{selected.abbreviations.join(", ") || "—"}</p>
              </div>
            </div>

            <div className="mb-3">
              <div className="mb-1 font-mono text-[9.5px] tracking-widest text-gowl-t6">{strings.buildRelations}</div>
              {relations && relations.length === 0 && <p className="text-[12px] text-gowl-t5">{strings.buildNoRelations}</p>}
              {relations?.map((relation, index) => (
                <div key={index} className="flex items-center justify-between border-b border-gowl-row py-1 text-[12px]">
            ...
            <div>
              <div className="mb-1 font-mono text-[9.5px] tracking-widest text-gowl-t6">{strings.buildUsage}</div>
              {usage && usage.length === 0 && <p className="text-[12px] text-gowl-t5">{strings.buildNoUsage}</p>}
              <ul>
                {usage?.map((fqn) => (
                  <li key={fqn} className="font-mono text-[11.5px] text-gowl-t2">
                    {fqn}
                  </li>
                ))}
              </ul>
