  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <p className="py-8 text-center text-[12.5px] text-gowl-t5">
            ...
            return (
              <div key={p.id} className="rounded-md border border-gowl-line bg-gowl-panel p-4">
                <div className="mb-2 flex items-start justify-between">
                  <div>
                    <span className="text-[13px] font-semibold text-gowl-t1">{p.term}</span>
                    <span className={`ml-2 rounded-full px-2 py-0.5 font-mono text-[8.5px] ${ss.bg} ${ss.text}`}>
                      {ss.label}
                    </span>
                  </div>
                  <span className="text-[11px] text-gowl-t5">{p.submittedBy} · {p.submittedAt}</span>
                </div>
                <p className="mb-2 text-[12px] leading-relaxed text-gowl-t3">{p.definition}</p>
                {p.reviewNote && (
                  <div className="rounded bg-gowl-panel-2 px-3 py-1.5 text-[11px] text-gowl-t5">
                    Review: {p.reviewNote}
                  </div>
                )}
                {p.status === "pending" && (
                  <div className="mt-3 flex gap-2">
                    <button type="button" className="rounded border border-gowl-ok-border bg-gowl-ok-bg px-2.5 py-1 text-[10.5px] text-gowl-ok">
                      Approve
                    </button>
                    <button type="button" className="rounded border border-gowl-line-2 px-2.5 py-1 text-[10.5px] text-gowl-t5">
                      Request info
                    </button>
                    <button type="button" className="rounded border border-gowl-bad-border bg-gowl-bad-bg px-2.5 py-1 text-[10.5px] text-gowl-bad">
                      Reject
                    </button>
                  </div>
                )}
