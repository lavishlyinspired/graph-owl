...
export function BusinessTab({ glossaryId }: { readonly glossaryId: string }) {
  ...
  return (
    <div>
      <p className="mb-4 text-[16.5px] text-gowl-t5">{strings.businessDescription}</p>
      {approved.length === 0 ? (
        <p className="text-[17px] text-gowl-t5">{strings.businessEmpty}</p>
      ) : (
        <div className="rounded-lg border border-gowl-line bg-gowl-panel">
          {approved.map((term) => (
            <div key={term.id} className="border-b border-gowl-row p-4 last:border-b-0">
              <div className="mb-1 text-[18px] font-semibold text-gowl-t1">{term.name}</div>
              <p className="text-[16.5px] text-gowl-t3">{term.definition || "—"}</p>
            </div>
          ))}
        </div>
      )}
