  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          {!ran ? (
            <p className="text-[12.5px] text-gowl-t5">Run validation to check the glossary against ontology rules and graph consistency.</p>
          ) : (
        ...
        <div className="space-y-2">
          {issues.length === 0 ? (
            <div className="rounded-md border border-gowl-ok-border bg-gowl-ok-bg p-6 text-center">
              <div className="text-[13px] font-semibold text-gowl-ok">All checks passed</div>
              <div className="mt-1 text-[11.5px] text-gowl-t5">No issues found in this glossary.</div>
            </div>
          ) : (
            issues.map((issue) => {
              ...
              return (
                <div key={issue.id} className="flex items-start gap-3 rounded-md border border-gowl-line bg-gowl-panel p-3">
                  <span className={`mt-0.5 flex-none h-5 w-5 rounded-full text-center text-[11px] leading-5 ${ss.bg} ${ss.text} font-bold`}>
                    {ss.icon}
                  </span>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-[12px] font-semibold text-gowl-t1">{issue.term}</span>
                      <span className="font-mono text-[10px] text-gowl-t6">{issue.rule}</span>
                    </div>
                    <p className="mt-0.5 text-[11.5px] text-gowl-t4">{issue.message}</p>
                  </div>
                </div>
              );
            })
