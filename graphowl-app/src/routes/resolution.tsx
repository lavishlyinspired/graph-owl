  ...
  return (
    <div className="flex h-full">
      <div className="flex-1 overflow-y-auto p-8">
        <h1 className="mb-1 text-[21px] font-semibold text-gowl-t1">{strings.resolutionTitle}</h1>
        <p className="mb-5 text-[12.5px] text-gowl-t5">{strings.resolutionDescription}</p>
        ...
        {checked.size > 0 && (
          <div className="mb-4 flex items-center gap-3 rounded-lg border border-gowl-accent-border bg-gowl-accent-bg px-4 py-2.5">
            <span className="text-[12.5px] text-gowl-t2">
              {checked.size} {strings.governSelectedSuffix}
            </span>
            <button
              type="button"
              onClick={() => setBulkAction("confirm")}
              className="rounded-md bg-gowl-accent px-3 py-1 text-[12px] font-semibold text-gowl-accent-on"
            >
            ...
            <button
              ...
              className="rounded-md border border-gowl-line-2 px-3 py-1 text-[12px] text-gowl-t2"
            >
        ...
        <div className="overflow-hidden rounded-lg border border-gowl-line bg-gowl-panel">
          <div className="grid grid-cols-[24px_1fr_1fr_80px_1.5fr] gap-3 border-b border-gowl-line bg-gowl-panel-2 px-4 py-2 font-mono text-[9.5px] tracking-wider text-gowl-t6">
            <span />
          ...
          {pending.length === 0 ? (
            <div className="p-6 text-[12.5px] text-gowl-t5">{strings.resolutionEmpty}</div>
          ) : (
            pending.map((entry) => (
              <div
                key={entry.id}
              ...
              >
                ...
                <button
                  ...
                  className="truncate text-left font-mono text-[11.5px] text-gowl-t2 hover:text-gowl-accent"
                >
                  {entry.target}
                </button>
                <span className="truncate font-mono text-[11.5px] text-gowl-t2">{entry.candidate}</span>
                <span className="font-mono text-[12px] text-gowl-t1">{entry.score.toFixed(2)}</span>
                <span className="truncate text-[12px] text-gowl-t5">{evidenceSummary(entry.evidence)}</span>
              </div>
            ))
          )}
        </div>
      ...
      {selected && (
        <div className="w-[380px] flex-none overflow-y-auto border-l border-gowl-line bg-gowl-panel p-5">
          <div className="mb-4 flex items-start justify-between">
            <div className="text-[13px] font-semibold text-gowl-t1">
          ...
          <div className="mb-1 font-mono text-[20px] text-gowl-t1">{selected.score.toFixed(2)}</div>
          <p className="mb-4 text-[12.5px] text-gowl-t3">{evidenceSummary(selected.evidence)}</p>
      ...
      {bulkAction && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50">
          <div className="w-[420px] rounded-lg border border-gowl-line bg-gowl-panel p-5">
            <div className="mb-3 text-[13px] font-semibold text-gowl-t1">{strings.resolutionBulkPreviewTitle}</div>
            <div className="mb-4 max-h-[240px] overflow-y-auto rounded-md border border-gowl-line-2">
              {Array.from(checked).map((id) => {
                const entry = pending.find((p) => p.id === id);
                return (
                  <div key={id} className="border-b border-gowl-row px-3 py-2 text-[12px] text-gowl-t2 last:border-b-0">
