...
- **`Router::nest("/prefix", inner)` cannot serve `inner`'s own root with a trailing slash — traced through axum 0.8.9's own source, not guessed.** `graph_owl_ui::router_next()` (Plan 122a) was `Router::new().route("/", get(serve_next))` nested at `/next`. `/next` (no slash) and `/next/anything` worked; `GET /next/` — the exact URL a browser requests for a console mounted at `/next` — silently served the *other* embed (`ui/dist`) instead, and stayed broken through several rebuilds because it looked like a stale-binary/zombie-process artifact (a real one of those was *also* in play the same session, which is what delayed the diagnosis). Root cause, in `axum::routing::path_router::path_for_nested_route`: an inner path of exactly `"/"` maps to the outer prefix *without* a trailing slash (`path == "/" → prefix`, special-cased before the general `format!("{prefix}{path}")` branch ever runs) — there is no inner path value that produces `prefix + "/"`. A `{*path}` wildcard doesn't cover the gap either, because matchit's catch-all requires at least one captured character, so it never matches an empty remainder. Net effect: the nested router's own trailing-slash root is **structurally unreachable** through `nest()`, for any inner router, regardless of how its fallback is wired.

  Two things this is *not*: it is not about `.fallback()` specifically (explicit routes have exactly the same gap, confirmed by reproducing it both ways), and it is not fixed by getting fallback-merge order right (`Router::merge`'s fallback semantics were traced too, and they work correctly — a nested router's *own* fallback is preserved and correctly prioritised over an outer one via a separate `fallback_router` structure; that is a real but different, only-theoretical footgun — `Router::merge` panics if both sides carry a custom fallback — worth avoiding on its own merits but orthogonal to this bug).

  **Fix**: don't rely on `nest()`'s path rewriting for a sub-router that needs to serve its own root. Give the inner router's routes the full prefix already baked in (`/next`, `/next/`, `/next/{*path}`, each a plain `.route(...)`) and `.merge()` it into the outer router instead of nesting it — `.merge()` unions route tables directly with no rewriting, so there's nothing for this gap to hide in. The handler then strips its own prefix manually rather than relying on axum's automatic nest-stripping.

  **Isolated router tests did not catch this.** `graph-owl-ui/tests/embed.rs` tested `/next/overview` and `/next/static/<asset>` against `router_next()` nested under a bare `Router::new()` — both passed, because neither hits the bare `/next/` root. Regression coverage needs the exact trailing-slash path tested explicitly, at both the isolated-router layer *and* against the real composed `app()` (a new `graph-owl-server/tests/next_embed.rs`, built via the existing `test_app()` Postgres fixture) — the full route table's merge order is a different scenario than one router tested alone, even though in this particular case the actual defect was in `nest()` itself, not in ordering.

- **A React `useState` setter called from a listener AntV G6 (or any canvas/WebGL library with its own event system) attached via raw `canvas.addEventListener` can run correctly and still never update state — no error, no warning, `flushSync` included.** Found live in Plan 122a A3's Explore screen: `instance.on("node:click", (evt) => { ... selectNode.current(found); })` — `found` was the right node (confirmed by logging it), the setter was reached (confirmed the same way), and React's own fiber state (read directly via `element[fiberKey]`, walking `.return` to the component and `.memoizedState` to the hook) stayed `null` afterward, every single time, across dozens of repeated tests. Wrapping the call in `flushSync` did not fix it either — ruling out a batching/scheduling explanation, not just a "forgot to flush" one. G6's `BehaviorController.forwardCanvasEvents` binds directly to the canvas DOM node with plain `addEventListener`, entirely outside React's synthetic event delegation; a state update queued from that context was observed to be silently dropped rather than merely delayed.

  **Fix**: don't call the setter from the library's own listener at all. Use the library only for what only it can do — hit-testing, resolving which element a pixel hit — write that result into a plain ref, and make the *actual* `setState` call from a genuine React event handler on an ancestor DOM node (e.g. `onClick` on the container div G6 renders into). The same underlying browser gesture that the library's own listener processed already bubbles up to that ancestor, so the ref is already populated by the time React's handler runs, and a setter called from inside a real React synthetic event was reliable every time this was tested. See `graphowl-app/src/graph/GraphCanvas.tsx`'s `pendingClick` ref for the shape of the fix.

  **A second, compounding bug rode along with this one and is easy to misdiagnose as the same thing**: one physical click fired *two* G6 events, not one — the element's own `node:click`/`edge:click`, then `canvas:click` immediately after, for the same gesture. G6's `forwardCanvasEvents` re-resolves the click target independently per underlying DOM event, and a `pointerup`-derived click and a separately synthesized `click` were not always resolving to the same target — the second fell through to the canvas and its deselect logic ran right after the element's own select. A one-shot guard flag (set the instant an element's own click is handled, consumed by the very next `canvas:click`) is the fix; without it, a "fixed" click handler still looks broken because the very next event undoes it.

  **Debugging this needs a synthetic `pointermove` before `pointerdown`/`pointerup`.** A `computer`-tool-style click with no prior hover does not always reach the library's own picking cache — send `pointermove` at the target coordinates first, then `pointerdown`/`pointerup`, when reproducing a canvas click programmatically for a repro or a test.

- **A flex child rendered *conditionally* next to a `flex-1` sibling needs `min-w-0` on the sibling, or it renders correctly and invisibly, off-screen.** `GraphCanvas`'s outer wrapper was `flex flex-1 flex-col` with no `min-width` override; a flexbox item's default `min-width` is `auto`, which — for a flex item containing further flex/block content — means "don't shrink below your content's natural width," not "shrink to fit like `width` does." Adding a sibling panel (`w-[352px]`) after clicking a node did not shrink the canvas to make room; the canvas kept its full width and the panel was pushed past the right edge of the viewport. It was not a rendering failure: React had genuinely committed the panel (confirmed by reading its `getBoundingClientRect()` directly — `left: 1512` on a `1512px`-wide viewport, `display: block`, `visibility: visible`, `opacity: 1`), and `document.body.innerText` contained its text. The state bug above was real and had to be fixed regardless, but stacked with this one, so fixing state-not-updating alone will *still* show nothing on screen — check `getBoundingClientRect()` on the missing element before assuming the state layer is still broken.
